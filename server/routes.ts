...
async function seedDatabase() {
  try {
    const existingUser = await storage.getUserByEmail("safeschool@gmail.com");
    if (!existingUser) {
      // Seed Super Admin
      await storage.createUser({
        email: "safeschool@gmail.com",
      ...
      await storage.createUser({
        ...
        password: await hashPassword("school123"),
        role: "school_admin",
        schoolId: school1.id
      });
      ...
      console.log("[seed] Users seeded: safeschool@gmail.com (super_admin), school@admin.com (school_admin)");
    }
  } catch (err) {
