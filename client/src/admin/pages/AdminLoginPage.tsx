...
const onboardingSlides = [
    {
        title: "Organized Pickup Management System",
        body: "Smart platform helping schools organize student pickup precisely, reducing congestion and ensuring top safety.",
    },
    {
        title: "Real-time Student Attendance Tracking",
        body: "Track attendance moment by moment, with real-time notifications to parents and comprehensive daily reports.",
    },
    {
        title: "Safer School Environment",
        body: "We protect your children through an advanced verification system and seamless coordination between management and parents.",
    },
];
...
export default function AdminLoginPage() {
    ...
    useEffect(() => {
        ...
        // Only auto-login if there's no school-admin session already active
        if (user && user.role?.toLowerCase() === 'school_admin') return;
        
        const saved = localStorage.getItem("safe_school_remember_me_admin");
        if (saved) {
            try {
    ...
    // Only redirect school admins — a super_admin in the admin session is a mis-login
                            ...
                            <form
                                onSubmit={form.handleSubmit((data) =>
                                    loginMutation.mutate(data, {
                                        onSuccess: () => {
                                            if (rememberMe) {
                                                localStorage.setItem("safe_school_remember_me_admin", JSON.stringify({
                                                    email: data.email,
            ...
            >
                ...
                >
                    ...
                    <h2 className="text-white text-2xl font-bold mb-3 leading-snug">
                        {t(onboardingSlides[activeSlide].title)}
                    </h2>
                    <p className="text-white/75 text-sm leading-relaxed max-w-xs">
                        {t(onboardingSlides[activeSlide].body)}
                    </p>
                </div>
